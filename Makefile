...
JAVA:=java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=1044 -Dlogback.configurationFile=${current_dir}/logback.xml -XX:+HeapDumpOnOutOfMemoryError -XX:+UnlockExperimentalVMOptions -XX:+UseCompactObjectHeaders
...
build-otp:
	cd ../OpenTripPlanner/ && \
		mvn clean package -Dmaven.test.skip -Dmaven.source.skip=true -P prettierSkip,checkstyleSkip && cp otp-shaded/target/otp-shaded-*-*.jar ../otp2-setup/otp.jar
